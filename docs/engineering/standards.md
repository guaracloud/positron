...
| `TEST-06` | Activated critical-policy harnesses MUST retain measured line, region, branch, and changed-code coverage baselines. Scheduled coverage MUST NOT regress below those baselines, and critical invariant paths require explicit test traceability. Mutation testing MAY be used as a focused diagnostic for unusually risky logic, but it is not a routine merge requirement. Numeric scores are detector evidence, never substitutes for behavioral proof. |
