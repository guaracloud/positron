...
| Stage | Blocking rule |
| --- | --- |
| `PR` | Runs a bounded deterministic suite on every pull request and merge-group revision: policy, architecture, host build, formatting, lints, tests, documentation, current-tree secrets, dependency policy, and advisories. All selected checks and required owner reviews MUST pass before merge. |
| `EXT` | Runs the slower cross-target, coverage, full-history secret, and deep supply-chain campaigns on the protected branch at the registry cadence. Failure blocks candidate qualification until fixed and rerun without forcing every development push through the campaign. |
...
| `RUST-02` | `EG-BUILD` / `PR+EXT+QUAL` | PR runs a clean locked host build; EXT and QUAL add the pinned target matrix, frozen release builds, overflow/profile checks, and reproducibility comparisons. | build plan, lock digest, profile report, reproducibility diff | Rust and Toolchain |
...
| `SEC-04` | `EG-SECRETS` / `PR+EXT+QUAL` | PR scans the current tree; EXT and QUAL add full-history and artifact canary scans across logs, errors, telemetry, diagnostics, evidence, binaries, and distributions. | scanner reports, canary manifest, redaction report | Security and Key Management |
| `SEC-05` | `EG-SUPPLY` / `PR+EXT+QUAL` | PR checks locked advisories and dependency review; EXT and QUAL add Vet policy and exact-artifact SBOM, reproducibility, signature, and provenance checks. | policy reports, SBOMs, repro diff, signature/provenance verification | Release Engineering |
...
| `TEST-06` | `EG-COVERAGE` / `EXT` | Pinned line, region, branch, and changed-code coverage enforces the measured critical-policy baselines on the scheduled campaign. Focused mutation analysis is optional diagnostic evidence for unusually risky logic, not a routine blocking campaign. | coverage files, ratchet and exclusion diff | Quality Engineering |
