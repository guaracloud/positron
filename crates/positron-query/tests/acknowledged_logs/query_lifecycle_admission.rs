...
use super::support::{TestClock, publish_lifecycle_at_catalog_for_test, zero_work_clock_service};
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("query-lifecycle-admission", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("query-catalog-generation-admission", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("query-retention-lease-domain", |fixture| {
...
fn resume_rejects_a_lifecycle_transition_at_marker_admission() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("resume-lifecycle-admission", |fixture| {
        ...
        let failure = with_catalog_publication_hook_after(
            0,
            |catalog| {
                publish_lifecycle_at_catalog_for_test(catalog, 3, 0xd6)
                    .expect("lifecycle transition publication");
            },
...
{
    QueryFixture::scoped("resume-expired-lease-admission", |fixture| {
        fixture.kernel.append_log("first", 20, 1)?;
        fixture.kernel.append_log("second", 21, 2)?;
        let initial = fixture.service(1)?;
        let planned = initial.plan_pipeline(
...
fn resume_rebases_over_unrelated_lease_marker_churn() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("resume-unrelated-lease-churn", |fixture| {
        fixture.kernel.append_log("first", 20, 1)?;
        fixture.kernel.append_log("second", 21, 2)?;
        let initial = fixture.service(1)?;
        let planned = initial.plan_pipeline(
        ...
        let events = with_catalog_publication_hook_after(
            0,
            move |catalog| {
                publish_snapshot_lease_marker_for_test(catalog, unrelated_identity, 0xd9)
...
fn resume_rejects_a_lifecycle_transition_during_marker_publication() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("resume-lifecycle-publication", |fixture| {
        fixture.kernel.append_log("first", 20, 1)?;
        fixture.kernel.append_log("second", 21, 2)?;
        let service = fixture.service(1)?;
        ...
        let failure = with_catalog_publication_hook_after(
            0,
            |catalog| {
                publish_lifecycle_at_catalog_for_test(catalog, 3, 0xd7)
