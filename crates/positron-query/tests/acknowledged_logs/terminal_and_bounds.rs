...
{
    QueryFixture::scoped("cancel", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("malformed", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("empty", |fixture| {
        let service = fixture.service(16)?;
        let query = service.plan_pipeline(
            fixture.context,
            "logs | range query_time -100 100 | limit 1",
...
fn response_header_exposes_every_effective_query_budget_limit() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("header-budget", |fixture| {
...
{
    QueryFixture::scoped("page-bounds", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("page-within-deadline", |fixture| {
        let service = zero_work_clock_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
            1,
...
fn scan_capacity_refusal_is_one_typed_non_complete_terminal() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("scan-capacity", |fixture| {
        fixture.kernel.append_log("accepted", 20, 1)?;
        let service = fixture.service(16)?;
        let query = service.plan_pipeline(
            fixture.context,
...
fn parsers_budgets_keys_and_cursor_bytes_enforce_exact_public_bounds() -> Result<(), Box<dyn Error>>
{
    ...
    QueryFixture::scoped("bounds", |fixture| {
        let service = fixture.service(16)?;
...
-> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("source-byte-bound", |fixture| {
        let service = fixture.service(16)?;
...
fn planning_memory_is_admitted_before_sql_and_pipeline_allocations() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("planning-memory-admission", |fixture| {
        let service = fixture.service(16)?;
        ...
        let search_failure = match service.plan_pipeline(
            ...
            Ok(_) => {
                return Err("search plan unexpectedly succeeded below its memory charge".into());
            },
            Err(failure) => failure,
        };
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("sql-incremental-token-memory", |fixture| {
        let service = fixture.service(16)?;
...
fn native_literal_peak_memory_is_admitted_before_validation_output() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("native-literal-peak-memory", |fixture| {
        let service = fixture.service(16)?;
        let values = (0..180)
            .map(|_| r#"string("x")"#)
            .collect::<Vec<_>>()
...
fn bounded_path_segments_transfer_exact_capacity() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("path-segment-capacity", |fixture| {
        let service = fixture.service(16)?;
        let path = (0..16)
...
fn sql_index_selector_transfers_both_owned_reservations() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("index-selector-capacity", |fixture| {
        let service = fixture.service(16)?;
...
{
    QueryFixture::scoped("sql-parenthesis-bound", |fixture| {
        let service = fixture.service(16)?;
        let literal = |depth| {
...
fn rejected_search_transfer_releases_parser_and_copy_reservations() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("search-transfer-rollback", |fixture| {
        let service = fixture.service(16)?;
        let before = fixture.kernel.authority.governor().inspect()?;
...
fn native_quoted_and_bytes_boundaries_share_closed_parser_errors() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("native-parser-boundaries", |fixture| {
        let service = fixture.service(16)?;
        for source in [
...
fn native_scalar_queries_transfer_exact_string_and_bytes_capacity() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("native-scalar-boundaries", |fixture| {
        let service = fixture.service(16)?;
...
{
    QueryFixture::scoped("pipeline-token-memory", |fixture| {
        let service = fixture.service(16)?;
...
pub(crate) fn with_query_fixture<T>(
    ...
    with_kernel_fixture_with_identity(instance.default_tenant_id(), label, &governance, |kernel| {
...
pub(crate) fn with_compaction_query_fixture<T>(
    ...
    let governance = instance.governance_fixture_for_test()?;
    with_compaction_kernel_fixture_with_identity(
        instance.default_tenant_id(),
        label,
        &governance,
        |kernel| {
...
impl QueryFixture<'_, '_, '_> {
    ...
    pub(crate) fn scoped_compaction<T>(
        ...
        with_compaction_query_fixture(label, action)
    }
}

impl<'fixture, 'kernel, 'catalog> QueryFixture<'fixture, 'kernel, 'catalog> {
    pub(crate) fn service(
        &self,
        batch_limit: u16,
    ) -> Result<QueryService<'kernel, 'catalog, '_>, Box<dyn Error>> {
