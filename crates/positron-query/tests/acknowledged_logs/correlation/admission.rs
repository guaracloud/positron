...
use super::super::support::{TestClock, zero_work_clock_service, zero_work_service};
use positron_domain::identity::TenantId;
use positron_domain::routing::{SignalKind, VirtualShardId};
use positron_kernel::{
    ActiveSegmentLedger, LedgerFailureCode, ResourceDimension, SegmentProtectionKey, SegmentScope,
    SnapshotLeaseId, WorkClass,
};
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("correlation-missing-log-identifier", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("correlation-ordinary-representation", |ordinary| {
        QueryFixture::scoped("correlation-overflow-representation", |overflow| {
            ...
            let ordinary_events = ordinary_service
                .execute(ordinary_service.plan_pipeline(
                    ordinary.context,
                    source,
                    super::budget(),
                )?)?
...
fn correlation_cursor_expiry_rejects_both_paired_snapshot_leases() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("correlation-paired-expiry", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("correlation-memory", |fixture| {
        ...
        assert!(
            held.usage(ResourceDimension::MemoryBytes)
                > baseline.usage(ResourceDimension::MemoryBytes)
        );
        assert_eq!(
...
fn correlation_cancellation_releases_both_paired_snapshot_leases() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("correlation-cancel-release", |fixture| {
