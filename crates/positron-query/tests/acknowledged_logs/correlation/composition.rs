...
fn sql_correlation_page_resume_binds_sidecar_to_projected_time_values() -> Result<(), Box<dyn Error>>
{
    ...
    QueryFixture::scoped("correlation-sql-projected-page-resume", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("correlation-filtered-page-resume", |fixture| {
        ...
        let cursor =
            |events: &[QueryEvent]| -> Result<positron_query::QueryCursor, Box<dyn Error>> {
                events
                    .iter()
                    .find_map(|event| match event {
                        ...
                        QueryEvent::Header(_) | QueryEvent::Batch(_) | QueryEvent::Terminal(_) => {
                            None
                        },
                    })
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("correlation-transformed-page-resume", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("correlation-cursor-output-bytes", |fixture| {
        ...
        let budget =
            QueryBudget::new(1_048_576, 1_024, 1_024, per_row_output_bytes, 1_048_576, 60)?;
        let first = service
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("schema-backed-correlation", |fixture| {
