...
fn text_predicates_skip_missing_and_non_text_bodies() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("query-text-predicate-body-kinds", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("query-transform-bounds", |fixture| {
        ...
        QueryFixture::scoped("query-transform-size", |oversized_fixture| {
            ...
            QueryFixture::scoped("query-transform-entry-bound", |entries_fixture| {
                let entries = (0..1_025)
...
fn transform_work_is_cumulative_and_cancellation_is_prompt() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("query-transform-work", |fixture| {
        fixture
            .kernel
        ...
        QueryFixture::scoped("query-transform-cancel", |cancelling| {
            cancelling
                .kernel
                .append_log(&format!("{{\"value\":{}}}", "7".repeat(128)), 20, 1)?;
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("query-transform-lifecycle", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("query-transform-escaped-search", |escaped| {
        ...
        QueryFixture::scoped("query-transform-object-search", |object| {
...
fn textual_predicates_must_follow_body_transform() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("query-transform-stage-order", |fixture| {
        let service = fixture.service(16)?;
        let budget = QueryBudget::new(1_048_576, 16, 16, 1_048_576, 1_048_576, 60)?;
        assert_eq!(
        service
...
fn temporal_exclusion_precedes_transform_parsing() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("query-transform-temporal-order", |fixture| {
...
fn transformed_predicate_cancellation_releases_transformed_memory() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("query-transform-predicate-cancellation", |fixture| {
        fixture
            .kernel
