...
fn public_queries_are_equivalent_before_after_and_after_restart() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped_compaction("compaction-public-query-equivalence", |fixture| {
        ...
        let scalar = r#"pipeline:v1 logs | range query_time -100 100 | filter body == "error-42" | limit 16"#;
        let contains = r#"pipeline:v1 logs | range query_time -100 100 | search body contains "error" | limit 16"#;
        ...
        let before_regex =
            query(fixture, regex).map_err(|error| format!("before regex: {error}"))?;
        ...
        let after_scalar =
            query(fixture, scalar).map_err(|error| format!("after scalar: {error}"))?;
        let after_contains =
        ...
        let malformed_after = malformed_code(fixture, malformed)
            .map_err(|error| format!("after malformed: {error}"))?;
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped_compaction("correlation-compaction-snapshot", |fixture| {
...
{
    QueryFixture::scoped_compaction("compaction-schema-query-equivalence", |fixture| {
                ...
                vec![(
                    ...
                    vec![
                        NativeLogAttribute::new(
                            ...
                            "priority".to_owned(),
                            vec![
                                positron_domain::value::CandidateAttributeValue::signed_integer(7),
                            ],
                        ),
                        NativeLogAttribute::new(
                            AttributeNamespace::Record,
