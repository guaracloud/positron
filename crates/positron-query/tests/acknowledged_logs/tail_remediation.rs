...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-total-limit-admission", |fixture| {
...
fn terminal_cursor_keeps_its_snapshot_lease_resumable() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-terminal-cursor-lease", |fixture| {
        let service = fixture.service(16)?;
        let budget = budget(1)?;
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-phase-header", |fixture| {
        let service = fixture.service(16)?;
        let query = service.plan_pipeline(
            fixture.context,
            "pipeline:v1 logs | range query_time -100 100 | limit all",
...
fn tail_cursor_preserves_planning_cpu_work() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-planning-cpu", |fixture| {
...
fn tail_nonmatching_record_persists_filter_cpu_in_safe_cursor() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-filter-cpu-progress", |fixture| {
        let service = super::support::stage_work_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
            16,
...
fn tail_resume_seeds_and_advances_durable_primary_usage() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-durable-usage", |fixture| {
        let service = super::support::stage_work_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
            16,
        ...
        let query_budget = QueryBudget::new(1_048_576, 16, 4, 1_048_576, 1_048_576, 60)?
            .with_cpu_work_units(1_024)?;
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-retained-memory-boundary", |fixture| {
...
fn total_limit_is_rejected_on_resume_before_history_or_lease_work() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-total-limit-resume", |fixture| {
        let service = fixture.service(16)?;
        let query = service.plan_pipeline(
            fixture.context,
            "pipeline:v1 logs | range query_time -100 100 | limit all",
...
fn explicit_ordering_is_rejected_for_historical_and_live_tail() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-explicit-order-admission", |fixture| {
        let service = fixture.service(16)?;
        for (source, start) in [
            (
...
fn traces_are_rejected_at_source_admission() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-traces-admission", |fixture| {
...
fn source_set_rejections_keep_the_public_failure_types() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-source-set-rejections", |fixture| {
        assert!(matches!(
...
fn catalog_failure_during_active_revalidation_is_store_unavailable() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-catalog-revalidation-failure", |fixture| {
        let service = fixture.service(16)?;
        let query = service.plan_pipeline(
            fixture.context,
            "pipeline:v1 logs | range query_time -100 100 | limit all",
...
fn mismatched_delivery_digest_fails_closed_during_replay() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-delivery-digest-mismatch", |fixture| {
        let service = fixture.service(16)?;
        let query = service.plan_pipeline(
            fixture.context,
            "pipeline:v1 logs | range query_time -100 100 | limit all",
...
fn resume_source_lease_publication_failure_is_store_unavailable() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-resume-publication-failure", |fixture| {
        let service = fixture.service(16)?;
        let query = service.plan_pipeline(
            fixture.context,
            "pipeline:v1 logs | range query_time -100 100 | limit all",
        ...
        let result = with_catalog_publication_fault_after(
...
{
    QueryFixture::scoped("tail-lease-rotation-retry", |fixture| {
        let service = fixture.service(16)?;
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-incomplete-history-replay", |fixture| {
        ...
        QueryFixture::scoped("tail-historical-idle-replay", |second_fixture| {
...
fn resume_rejects_a_source_snapshot_identity_switch() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-resume-snapshot-identity", |fixture| {
        let service = fixture.service(16)?;
        let query = service.plan_pipeline(
            fixture.context,
            "pipeline:v1 logs | range query_time -100 100 | limit all",
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-resume-binding-count", |fixture| {
...
fn live_scan_observes_cancellation_before_accounting_scanned_bytes() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-scan-cancel-observer", |fixture| {
...
fn cleanup_ranking_inspects_each_public_terminal_failure_code() -> Result<(), Box<dyn Error>> {
    {
        QueryFixture::scoped("tail-cleanup-code-lag", |fixture| {
    ...
    {
        QueryFixture::scoped("tail-cleanup-code-budget", |fixture| {
            let service = super::support::stage_work_service(
                fixture.kernel.authority.governor(),
                fixture.kernel.ledger()?,
                16,
    ...
    {
        QueryFixture::scoped("tail-cleanup-code-expired", |fixture| {
    ...
    {
        QueryFixture::scoped("tail-cleanup-code-cancelled", |fixture| {
            let service = fixture.service(16)?;
            let query = service.plan_pipeline(
                fixture.context,
                "pipeline:v1 logs | range query_time -100 100 | limit all",
    ...
    {
        QueryFixture::scoped("tail-cleanup-code-disconnected", |fixture| {
            let service = fixture.service(16)?;
            let query = service.plan_pipeline(
                fixture.context,
                "pipeline:v1 logs | range query_time -100 100 | limit all",
...
fn resume_rejects_a_delivery_cursor_with_a_late_sequence() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-resume-delivery-sequence", |fixture| {
        let service = fixture.service(16)?;
        let query = service.plan_pipeline(
            fixture.context,
            "pipeline:v1 logs | range query_time -100 100 | limit all",
...
{
    QueryFixture::scoped("tail-secondary-only-admission", |fixture| {
        let second = ActiveSegmentLedger::open(
            fixture.kernel.authority,
            fixture.kernel.catalog_for_test(),
            positron_kernel::SegmentScope::new(
...
{
    QueryFixture::scoped("tail-no-total-limit-live-transition", |fixture| {
...
fn historical_tail_orders_records_across_multiple_committed_blocks() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-historical-ordering", |fixture| {
...
{
    QueryFixture::scoped("tail-unbounded-transition", |fixture| {
...
{
    QueryFixture::scoped("tail-multi-shard-resume-accounting", |fixture| {
        let second = ActiveSegmentLedger::open(
            fixture.kernel.authority,
            fixture.kernel.catalog_for_test(),
            positron_kernel::SegmentScope::new(
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-historical-decoded-budget", |fixture| {
        ...
        let query = service.plan_pipeline(
            fixture.context,
            "pipeline:v1 logs | range query_time -100 100 | limit all",
            QueryBudget::new(1_048_576, 1, 4, 1_048_576, 1_048_576, 60)?
                .with_cpu_work_units(1_024)?,
        )?;
...
fn historical_output_rows_refusal_never_publishes_a_prefix() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-historical-output-rows", |fixture| {
...
{
    QueryFixture::scoped("tail-late-delivery-cursor", |fixture| {
        fixture.kernel.append_log("first", 1, 1)?;
        fixture.kernel.append_log("second", 2, 2)?;
        let service = fixture.service(16)?;
        let query = service.plan_pipeline(
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("tail-drop-release-fault-loop", |fixture| {
        let service = fixture.service(16)?;
        let source = "pipeline:v1 logs | range query_time -100 100 | limit all";
        let budget = budget(1)?;
        for _ in 0..65 {
