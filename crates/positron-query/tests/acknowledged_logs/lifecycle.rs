...
{
    QueryFixture::scoped("lease-reclaim", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("stream-resource-ownership", |fixture| {
...
{
    QueryFixture::scoped("matcher-stream-resource-ownership", |fixture| {
        fixture.kernel.append_log("one", 20, 1)?;
...
fn repeated_logical_plan_access_borrows_one_admitted_plan() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("logical-plan-access-ownership", |fixture| {
        let service = super::support::zero_work_clock_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
            1,
...
{
    QueryFixture::scoped("pre-stream-resource-ownership", |fixture| {
        let service = super::support::zero_work_clock_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
            1,
...
{
    QueryFixture::scoped("pre-stream-usage-publication-fault", |fixture| {
...
fn query_pre_stream_usage_failure_is_typed_and_releases_admission() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("query-pre-stream-usage-publication-fault", |fixture| {
        fixture.kernel.append_log("one", 20, 1)?;
...
fn paged_drop_retains_only_after_a_resume_cursor_is_delivered() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("paged-drop", |fixture| {
        fixture.kernel.append_log("one", 20, 1)?;
...
fn paged_drop_before_header_delivery_reclaims_every_snapshot_lease() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("paged-drop-before-header", |fixture| {
        let service = super::support::zero_work_clock_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
            1,
...
fn observed_paged_completion_reclaims_every_snapshot_lease() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("paged-complete-reclaim", |fixture| {
        let service = super::support::zero_work_clock_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
            1,
...
fn paged_drop_after_batch_before_terminal_replays_the_same_batch() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("paged-batch-ambiguity", |fixture| {
        fixture.kernel.append_log("one", 20, 1)?;
        let service = super::support::zero_work_clock_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
...
fn observed_paged_completion_makes_its_cursor_unavailable() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("paged-complete-terminal", |fixture| {
        let service = super::support::zero_work_clock_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
            1,
...
fn observed_paged_incomplete_is_terminal_and_not_resumable() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("paged-incomplete-terminal", |fixture| {
        fixture.kernel.append_log("one", 20, 1)?;
        let service = super::support::stage_work_clock_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("cancel-truth", |fixture| {
        fixture.kernel.append_log("one", 20, 1)?;
        fixture.kernel.append_log("two", 21, 2)?;
        let service = super::support::stage_work_clock_service(
            fixture.kernel.authority.governor(),
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("cancel-delivery", |fixture| {
        fixture.kernel.append_log("one", 20, 1)?;
        fixture.kernel.append_log("two", 21, 2)?;
        let service = super::support::zero_work_clock_service(
            fixture.kernel.authority.governor(),
