...
fn logfmt_typed_quoted_duplicate_and_limit_contract_is_public() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("query-logfmt-edge-values", |fixture| {
...
fn logfmt_malformed_and_input_limits_are_stable() -> Result<(), Box<dyn Error>> {
    ...
    .enumerate()
    {
        QueryFixture::scoped(&format!("query-logfmt-malformed-{index}"), |fixture| {
    ...
    let entries = (0..1_025)
        ...
        .collect::<Vec<_>>()
        .join(" ");
    QueryFixture::scoped("query-logfmt-entry-limit", |fixture| {
        ...
        QueryFixture::scoped("query-logfmt-input-limit", |fixture| {
            fixture
                .kernel
