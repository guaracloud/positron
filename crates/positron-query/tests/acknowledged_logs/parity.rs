...
use super::support::{TemporaryRoots, with_kernel_fixture_with_identity};
...
fn pipeline_and_sql_require_the_same_explicit_bounded_temporal_range() -> Result<(), Box<dyn Error>>
{
    ...
    with_kernel_fixture_with_identity(
        ...
        |fixture| {
            ...
            let pipeline = service.plan_pipeline(
                context,
                "logs | range event_time 10 30 | limit 16",
                budget,
            )?;
            let sql = service.plan_sql(
        context,
...
fn aggregate_ordering_is_rejected_by_both_frontends() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("aggregate-order-parity", |fixture| {
...
fn sql_and_pipeline_edge_rejections_keep_their_public_classes() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("parser-edge-parity", |fixture| {
        let service = fixture.service(16)?;
        let budget = QueryBudget::new(1_048_576, 16, 16, 1_048_576, 1_048_576, 60)?;
        for source in [
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("namespace-parity", |fixture| {
        let service = fixture.service(16)?;
        let budget = QueryBudget::new(1_048_576, 16, 16, 1_048_576, 1_048_576, 60)?;
        ...
        |fixture| {
            ...
            for source in [
                ...
                let failure = match service.plan_sql(context, source, budget) {
                    Ok(_) => {
                        return Err(format!("unsupported SQL form was accepted: {source}").into());
                    },
                    Err(failure) => failure,
                };
                assert_eq!(
