...
-> Result<(), Box<dyn Error>> {
    with_cursor_fixture(|fixture| {
        ...
        *byte ^= 1;
        ...
        let catalog_mismatch =
            rewritten_cursor(fixture, |bytes| bytes[88] ^= 1, b"query-cursor-v4")?;
        assert_eq!(
            fixture
                .service()
...
fn authenticated_cursor_semantics_versions_and_domain_are_fail_closed() -> Result<(), Box<dyn Error>>
{
    with_cursor_fixture(|fixture| {
        ...
        with_cursor_fixture(|source_fixture| {
        ...
        for axis in [2_u8, 3_u8] {
            with_cursor_fixture(|axis_fixture| {
...
struct CursorFixture<'fixture, 'kernel, 'catalog> {
    kernel: &'fixture mut KernelFixture<'kernel, 'catalog>,
...
fn with_cursor_fixture<T>(
    action: impl for<'fixture, 'kernel, 'catalog> FnOnce(
        &mut CursorFixture<'fixture, 'kernel, 'catalog>,
    ...
    let governance = instance.governance_fixture_for_test()?;
    let fixture_governance = governance.clone();
    with_kernel_fixture_with_identity(
        instance.default_tenant_id(),
...
impl<'fixture, 'kernel, 'catalog> CursorFixture<'fixture, 'kernel, 'catalog> {
    fn service(&self) -> QueryService<'kernel, 'catalog, '_> {
