...
fn typed_projection_bytes_obey_the_exact_output_budget() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("typed-projection-bytes", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("output-budget-physical-attempt", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("event-time-projection", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("lossless-temporal-projection", |fixture| {
        fixture.kernel.append_logs(
            vec![
                (
                    Some(20),
...
-> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("event-time-grouping", |fixture| {
        fixture.kernel.append_logs(
            vec![
                (
                    Some(20),
...
fn event_time_range_excludes_records_without_event_time() -> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("event-time-range-missing", |fixture| {
        fixture.kernel.append_logs(
            vec![
                (
                    Some(20),
...
-> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("event-time-ordering-axis", |fixture| {
        fixture.kernel.append_logs(
            vec![
                (
                    Some(0),
...
fn empty_string_body_equality_remains_distinct_from_a_missing_body() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("empty-body-equality", |fixture| {
...
fn quoted_pipeline_literals_preserve_stage_delimiters_as_body_data() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("quoted-stage-delimiter", |fixture| {
...
fn escaped_pipeline_literals_round_trip_quote_backslash_and_pipe() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("escaped-body-literal", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("native-body-values", |fixture| {
        let candidates = vec![
        ...
        let query = service.plan_pipeline(
            ...
            QueryBudget::new(1_048_576, 12, 12, 1_048_576, 1_048_576, 60)?
                .with_cpu_work_units(16)?,
        )?;
        let records = service
            .execute(query)?
...
fn projection_preserves_missing_and_native_body_values() -> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("native-body-projection", |fixture| {
        let expected = [
            CandidateAttributeValue::boolean(false)
                .validate_log_body(ValueLimitProfile::release_1_system_maximum())?,
...
fn typed_body_equality_accepts_every_native_literal_without_coercion() -> Result<(), Box<dyn Error>>
{
    ...
    QueryFixture::scoped("typed-body-literals", |fixture| {
        let candidates = [
...
-> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("typed-attribute-query", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("attribute-projection-memory", |fixture| {
        fixture.kernel.append_attribute_logs(
            vec![(
                Some(10),
        ...
        let exact = service.plan_pipeline(
            fixture.context,
            source,
            QueryBudget::new(1_048_576, 1, 1, 1_048_576, exact_peak, 60)?
                .with_cpu_work_units(16)?,
        )?;
        let exact_events = service.execute(exact)?.collect::<Vec<_>>();
        assert!(
...
-> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("typed-attribute-grouping", |fixture| {
        fixture.kernel.append_attribute_logs(
            vec![
                (
                    Some(10),
...
fn version_one_equality_rejects_untyped_literal_syntax() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("native-filter-boundary", |fixture| {
        let service = super::support::zero_work_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
            16,
...
fn version_one_rejects_malformed_or_noncanonical_typed_literals() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("native-literal-errors", |fixture| {
        let service = super::support::zero_work_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
            16,
        ...
        let source = format!(
            "pipeline:v1 logs | range query_time 0 100 | filter body == {too_deep} | limit 1"
        );
        let failure = service
            .plan_pipeline(fixture.context, &source, budget)
            .err()
...
fn version_one_rejects_malformed_attribute_paths_and_selectors() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("attribute-path-errors", |fixture| {
        let service = super::support::zero_work_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
            16,
...
fn typed_count_bytes_obey_the_exact_output_budget() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("typed-count-bytes", |fixture| {
...
fn result_header_preserves_both_total_order_directions() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("typed-order-header", |fixture| {
...
fn grouped_schema_describes_native_keys_and_unsigned_counts() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("typed-group-schema", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("typed-projection-digest", |fixture| {
        ...
        let event_time =
            "pipeline:v1 logs | range query_time -100 100 | project event_time | limit 1";
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("streamed-result-digest", |fixture| {
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("same-block-total-order", |fixture| {
        fixture.kernel.append_log_bodies(
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("default-sort-work", |fixture| {
...
{
    QueryFixture::scoped("observed-filter-work-ledger", |fixture| {
        fixture.kernel.append_logs(
            vec![
                (
                    Some(10),
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("typed-peak-memory", |fixture| {
        fixture.kernel.append_log("later", 20, 1)?;
        fixture.kernel.append_log("earlier", 10, 2)?;
        let service = QueryService::with_runtime(
            fixture.kernel.authority.governor(),
...
fn default_total_order_observes_cancellation_inside_sorting() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("default-sort-cancel", |fixture| {
...
fn cancellation_interrupts_substantial_default_sort_work() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("default-sort-mid-cancel", |fixture| {
        ...
        let events = std::thread::scope(|scope| -> Result<_, Box<dyn Error>> {
            let service = &service;
            let worker =
                scope.spawn(move || service.execute(query).map(Iterator::collect::<Vec<_>>));
...
{
    QueryFixture::scoped("typed-operator-combinations", |fixture| {
        let service = super::support::zero_work_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
            16,
...
fn grouped_count_emits_deterministic_typed_intrinsic_rows() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("typed-grouped-count", |fixture| {
...
fn grouped_key_comparisons_consume_the_exact_cumulative_work_budget() -> Result<(), Box<dyn Error>>
{
    ...
    QueryFixture::scoped("group-comparison-work", |fixture| {
        fixture.kernel.append_log_bodies(
            vec![
...
fn group_key_construction_charges_large_native_values_before_lookup() -> Result<(), Box<dyn Error>>
{
    ...
    QueryFixture::scoped("group-key-construction-work", |fixture| {
        fixture.kernel.append_log_bodies(
...
fn cancellation_is_polled_during_native_group_key_construction() -> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("group-key-construction-cancel", |fixture| {
        fixture.kernel.append_log_bodies(
            vec![Some(CandidateAttributeValue::string("x".repeat(4_096)))],
            20,
            1,
...
fn deep_native_predicate_work_exhausts_before_any_result_prefix() -> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("deep-native-predicate-work", |fixture| {
...
fn deep_native_body_projection_defers_recursive_work_to_output() -> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("deep-native-projection-work", |fixture| {
        let mut body = CandidateAttributeValue::null();
        for _ in 0..8 {
            body = CandidateAttributeValue::array(vec![body]);
        }
...
fn deep_native_output_and_digest_share_the_cumulative_cpu_budget() -> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("deep-native-output-work", |fixture| {
        let mut body = CandidateAttributeValue::null();
        for _ in 0..8 {
            body = CandidateAttributeValue::array(vec![body]);
        }
...
fn deep_native_digest_traversal_is_cumulatively_metered() -> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("deep-native-digest-work", |fixture| {
        let mut body = CandidateAttributeValue::null();
        for _ in 0..8 {
            body = CandidateAttributeValue::array(vec![body]);
        }
...
fn cancellation_interrupts_deep_native_digest_before_batch_delivery() -> Result<(), Box<dyn Error>>
{
    ...
    QueryFixture::scoped("deep-native-digest-cancel", |fixture| {
        let mut body = CandidateAttributeValue::null();
        for _ in 0..8 {
            body = CandidateAttributeValue::array(vec![body]);
        }
...
fn deep_attribute_path_projection_is_cumulatively_metered() -> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("deep-attribute-projection-work", |fixture| {
        ...
        let source =
            format!("pipeline:v1 logs | range query_time -100 100 | project {path} | limit 1");
        let query = service.plan_pipeline(
            fixture.context,
            &source,
...
-> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("deep-attribute-projection-cancel", |fixture| {
        let mut value = CandidateAttributeValue::null();
        let mut path = r#"record["payload"]"#.to_owned();
        for index in (0..8).rev() {
            let key = format!("k{index}");
...
-> Result<(), Box<dyn Error>> {
    ...
    QueryFixture::scoped("schema-aware-query", |fixture| {
        ...
        let budget = QueryBudget::new(1_048_576, 1, 1, 1_048_576, 1_048_576, 60)?
            .with_cpu_work_units(128)?;
        ...
        let Some(QueryEvent::Terminal(QueryTerminal::Complete(pruned_stats))) = pruned.last()
        else {
        ...
        let active_budget = QueryBudget::new(1_048_576, 3, 1, 1_048_576, 1_048_576, 60)?
            .with_cpu_work_units(128)?;
        ...
        let structural_budget = QueryBudget::new(1_048_576, 4, 1, 1_048_576, 1_048_576, 60)?
            .with_cpu_work_units(128)?;
...
fn cancellation_interrupts_grouping_and_releases_query_resources() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("typed-group-cancellation", |fixture| {
        for identity in 1_u8..=8 {
            fixture.kernel.append_log(
...
{
    QueryFixture::scoped("typed-stage-cancellation", |fixture| {
...
fn cancellation_after_scan_returns_one_terminal_without_output() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("cancel-after-scan", |fixture| {
        fixture.kernel.append_log("one", 20, 1)?;
        let meter = CancellingOperatorCallMeter::shared_for_stage(
            positron_query::QueryWorkStage::ScanDecode,
            2,
...
fn operator_wall_budget_is_checked_before_output() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("typed-operator-wall", |fixture| {
        fixture.kernel.append_log("one", 20, 1)?;
...
fn post_digest_wall_expiry_never_claims_an_unqueued_batch() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("post-digest-wall", |fixture| {
        fixture.kernel.append_log("one", 20, 1)?;
        let service = super::support::zero_work_clock_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("pipeline-full-text-search", |fixture| {
...
fn body_search_memory_peak_includes_matcher_and_retained_rows() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("pipeline-search-memory-peak", |fixture| {
        fixture.kernel.append_log("request timed out", 20, 1)?;
        let service = super::support::zero_work_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
...
fn body_regex_memory_peak_includes_automaton_and_retained_rows() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("pipeline-regex-memory-peak", |fixture| {
        fixture.kernel.append_log("request timed out", 20, 1)?;
        let service = super::support::zero_work_service(
            fixture.kernel.authority.governor(),
            fixture.kernel.ledger()?,
        ...
        let source = r#"pipeline:v1 logs | range query_time -100 100 | search body =~ "timed.*out" | limit 1"#;
...
{
    QueryFixture::scoped("schema-text-budget", |fixture| {
        let schema = fixture
            .kernel
...
fn schema_backed_regex_search_keeps_index_and_dfa_work_bounded() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("schema-regex-budget", |fixture| {
        let schema = fixture
            .kernel
            .append_indexed_text_logs(vec!["prefix needle suffix"], 1)?;
        let service = QueryService::with_runtime(
...
-> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("pipeline-regex-search", |fixture| {
...
fn body_search_polls_cancellation_during_matching() -> Result<(), Box<dyn Error>> {
    QueryFixture::scoped("pipeline-search-cancellation", |fixture| {
