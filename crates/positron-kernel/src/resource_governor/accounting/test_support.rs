...
impl GovernorConfiguration {
    pub(in crate::resource_governor) fn payload_addresses_for_test(&self) -> [usize; 14] {
        [
            self.state.tenant_quotas.as_ptr() as usize,
            self.state.tenant_fair_capacities.as_ptr() as usize,
            self.state.recovery_tenant_shared_fair.as_ptr() as usize,
            self.state.recovery_tenant_pool_fair.as_ptr() as usize,
...
impl GovernorInner {
    pub(in crate::resource_governor) fn payload_addresses_for_test(&self) -> [usize; 14] {
        ...
        [
            state.tenant_quotas.as_ptr() as usize,
            state.tenant_fair_capacities.as_ptr() as usize,
            state.recovery_tenant_shared_fair.as_ptr() as usize,
            state.recovery_tenant_pool_fair.as_ptr() as usize,
