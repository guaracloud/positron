...
pub(super) fn injected_partial_write_length(
    ...
    #[cfg(any(test, fuzzing))]
    if CATALOG_FAULT.with(|fault| fault.get()) == Some(_event) {
        CATALOG_FAULT.with(|fault| fault.set(None));
...
#[cfg(any(test, fuzzing))]
thread_local! {
    static CATALOG_FAULT: std::cell::Cell<Option<CatalogFileEvent>> = const { std::cell::Cell::new(None) };
}

#[cfg(any(test, fuzzing))]
pub(crate) fn with_catalog_fault<T>(event: CatalogFileEvent, action: impl FnOnce() -> T) -> T {
    CATALOG_FAULT.with(|fault| {
        let previous = fault.replace(Some(event));
        let result = action();
        fault.set(previous);
        result
    })
