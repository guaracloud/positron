...
fn recovery_rejects_the_wrong_segment_wrapping_key() -> Result<(), Box<dyn Error>> {
    ...
    ledger.append(PreparedStoreBlock::new(b"protected".to_vec())?)?;
    drop(ledger);
...
fn recovery_rejects_committed_frame_corruption_and_truncation() -> Result<(), Box<dyn Error>> {
    for truncate in [false, true] {
        ...
        let receipt = ledger.append(PreparedStoreBlock::new(b"durable".to_vec())?)?;
        drop(ledger);
...
fn recovery_discards_only_bytes_after_the_authenticated_frontier() -> Result<(), Box<dyn Error>> {
    ...
    let receipt = ledger.append(PreparedStoreBlock::new(b"frontier-bounded".to_vec())?)?;
    drop(ledger);
    let path = active_segment(fixture.root.path(), receipt.segment_id());
...
fn recovery_rejects_frontier_authenticator_corruption() -> Result<(), Box<dyn Error>> {
    ...
    let receipt = ledger.append(PreparedStoreBlock::new(b"authenticated".to_vec())?)?;
    drop(ledger);
...
-> Result<(), Box<dyn Error>> {
    for mutation in 0..3 {
        ...
        let receipt = ledger.append(PreparedStoreBlock::new(b"frontier-shape".to_vec())?)?;
        drop(ledger);
        let path = active_frontier(fixture.root.path(), receipt.segment_id());
        match mutation {
...
fn recovery_rejects_an_unknown_segment_header_version() -> Result<(), Box<dyn Error>> {
    ...
    let receipt = ledger.append(PreparedStoreBlock::new(b"versioned".to_vec())?)?;
    drop(ledger);
    let path = active_segment(fixture.root.path(), receipt.segment_id());
    let mut file = OpenOptions::new().write(true).open(path)?;
...
fn sealed_segments_reject_bytes_beyond_their_frontier() -> Result<(), Box<dyn Error>> {
    ...
    let receipt = ledger.append(PreparedStoreBlock::new(b"sealed-exact".to_vec())?)?;
    ledger.seal()?;
...
-> Result<(), Box<dyn Error>> {
    for mutation in 0..3 {
        ...
        let receipt = ledger.append(PreparedStoreBlock::new(b"safe-path".to_vec())?)?;
        drop(ledger);
...
fn recovery_rejects_a_frontier_present_in_both_lifecycle_directories() -> Result<(), Box<dyn Error>>
{
    ...
    let receipt = ledger.append(PreparedStoreBlock::new(b"one-frontier".to_vec())?)?;
    drop(ledger);
...
fn recovery_rejects_unsafe_temporary_and_unpublished_artifact_shapes() -> Result<(), Box<dyn Error>>
{
    ...
    let segment = temporary_ledger
        .append(PreparedStoreBlock::new(b"temporary-shape".to_vec())?)?
        .segment_id();
