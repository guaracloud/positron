...
-> Result<(), Box<dyn Error>> {
    ...
    let committed = ledger.append(prepared(8, b"snapshot-capacity".to_vec())?)?;
    ...
    assert_eq!(
        ledger
            .append(prepared(8, b"snapshot-conflict".to_vec())?)
    ...
    assert_eq!(
        ledger
            .append(prepared(9, b"new-work".to_vec())?)
...
-> Result<(), Box<dyn Error>> {
    ...
    let committed = ledger.append(prepared(9, b"committed-before-shutdown".to_vec())?)?;
    let active = root.path().join("segments/active");
    let before = directory_file_bytes(&active)?;
    let usage = authority.governor().inspect()?.outstanding_total();
    authority.begin_shutdown()?;
    assert_eq!(
        ledger.append(prepared(9, b"committed-before-shutdown".to_vec())?)?,
        committed
    );
    assert_eq!(
        ledger
            .append(prepared(9, b"changed".to_vec())?)
            .expect_err("conflict")
            .code(),
        LedgerFailureCode::IdempotencyConflict
    );
    assert_eq!(
        ledger
            .append(prepared(10, b"new".to_vec())?)
...
fn retained_ledger_memory_is_bounded_before_append_mutation() -> Result<(), Box<dyn Error>> {
    ...
    ledger.append(prepared(4, vec![0x61; 600_000])?)?;
    assert_eq!(
        ledger
...
-> Result<(), Box<dyn Error>> {
    ...
    assert_ne!(
        recovered
            .append(prepared(7, b"post-crash".to_vec())?)?
            .segment_id(),
...
fn explicit_seal_publishes_the_same_bytes_as_an_immutable_segment() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        ActiveSegmentLedger::open(&authority, &catalog, scope, key())?
            .append(prepared(8, b"sealed-block".to_vec())?)?,
        committed
    );
    Ok(())
}
