...
fn acknowledged_block_is_visible_and_survives_reopen() -> Result<(), Box<dyn Error>> {
    ...
    let receipt = ledger.append(prepared(scope, 1, b"canonical-log-block".to_vec())?)?;
...
-> Result<(), Box<dyn Error>> {
    ...
    let first = ledger.append(PreparedStoreBlock::new(scope, first_id, b"same".to_vec())?)?;
    let retry = ledger.append(PreparedStoreBlock::new(scope, first_id, b"same".to_vec())?)?;
    let second = ledger.append(PreparedStoreBlock::new(scope, second_id, b"same".to_vec())?)?;
    ...
    let retry = reopened.append(PreparedStoreBlock::new(scope, first_id, b"same".to_vec())?)?;
    assert_eq!(retry, first);
...
fn cancelled_work_is_rejected_before_resource_admission_and_mutation() -> Result<(), Box<dyn Error>>
{
    ...
    let failure = ledger
        .append_cancellable(prepared(scope, 2, b"cancelled".to_vec())?, &cancellation)
...
fn retrying_the_same_canonical_block_does_not_admit_a_duplicate() -> Result<(), Box<dyn Error>> {
    ...
    let original = ledger.append(prepared(scope, 3, b"retry-safe".to_vec())?)?;
    ledger.append(prepared(scope, 4, b"later-block".to_vec())?)?;
    let retry = ledger.append(prepared(scope, 3, b"retry-safe".to_vec())?)?;
    assert_eq!(retry, original);
    assert_eq!(ledger.snapshot()?.blocks().len(), 2);
    drop(ledger);

    let reopened = ActiveSegmentLedger::open(&authority, &catalog, scope, wrapping_key())?;
    let after_restart = reopened.append(prepared(scope, 3, b"retry-safe".to_vec())?)?;
