...
use positron_ingest::{
    ...
    IngestPolicy, IngestPolicyAuthority, LogIngest, NativeLogCandidate, OtlpLogsReceiver,
    PolicyAction, PolicyPredicate, PolicyRule, PolicyTarget,
};
use positron_kernel::{
...
#[test]
fn admitted_request_keeps_one_generation_snapshot_across_groups_during_publication()
-> Result<(), Box<dyn Error>> {
    ...
    let authority = IngestPolicyAuthority::new(old);
    let admitted = authority.snapshot()?;
    ...
    authority.publish(body_policy(32, PolicyAction::Remove(PolicyTarget::body()))?)?;
    ...
    for ledger in &ledgers {
        ...
        assert_eq!(
            result.records()[0]
                .body()
                .and_then(|body| body.policy_marker()),
    ...
    let new_result = ingest_and_scan(&refreshed_fixture, new_batch, current.policy(), 82)?;
    assert_eq!(
        new_result.records()[0]
            .body()
            .and_then(|body| body.policy_marker()),
