...
impl TemporaryRoot {
    pub fn new() -> Result<Self, std::io::Error> {
        ...
        let path = std::env::temp_dir().join(format!(
            "positron-log-store-test-{}-{sequence}",
            std::process::id()
        ));
        fs::create_dir(&path)?;
        Ok(Self(path))
...
fn add(left: ResourceAmounts, right: ResourceAmounts) -> Result<ResourceAmounts, Box<dyn Error>> {
    let value = |dimension| -> Result<u64, Box<dyn Error>> {
        left.get(dimension)
            ...
            .ok_or_else(|| "log-store test resource capacity overflow".into())
    };
