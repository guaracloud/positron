//! Executable cryptographic known-answer and fail-closed harness checks.
...
pub(super) fn run() -> Result<&'static str, XtaskError> {
...
fn verify_known_answer_vector() -> Result<(), XtaskError> {
    if Sha256::digest(b"abc").as_slice() != SHA256_ABC {
